...
//
//
// Related Topics 设计 哈希表 数据流 有序集合 堆（优先队列） 👍 165 👎 0
...
public class Medium2034 {
    ...
    public static void main(String[] args) {
        Medium2034 medium = new Medium2034();
        medium.update(1, 10);
        medium.update(2, 5);
        System.out.println(medium.maximum());
    }
    ...
    public void update(int timestamp, int price) {
        if (map.containsKey(timestamp)) {
            ...
            if (treeMap.containsKey(old)){
                treeMap.merge(old, -1, Integer::sum);
                if (treeMap.get(old) == 0){
                    treeMap.remove(old);
                }
            }
        }
        map.put(timestamp, price);
        treeMap.merge(price, 1, Integer::sum);
        if (index < timestamp) {
            index = timestamp;
        }
